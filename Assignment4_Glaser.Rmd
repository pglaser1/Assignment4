...
In the code its important to still call for the primary type as labels for the number of arrests for each that are sorted in descending order for 2016. The top of the list is narcotics arrests in 2016.

```{sql connection = con}

SELECT primary_type, count(*) AS number_arrests
FROM crime
...
This shows that the most arrests happened at 19 (3843) and the least at 5 (517).

```{sql connection = con}

SELECT EXTRACT(HOUR FROM date) AS hour_of_day, count(*) AS number_arrests
FROM crime
WHERE year = 2016 AND arrest = TRUE
...
2001 had the highest number of homicide arrests at 431 and 2025 has had the least so far at 100 but for complete years the lowest is 2014 at 199.

```{sql connection = con}

SELECT count(*) AS number_arrests, year
FROM crime
...
District 11 had the highest number in both 2015 and 2016 followed by district 7 in 2015. The lowest was district 31.

```{sql connection = con}

SELECT district, count(*) AS number_arrests, year
FROM crime
...
In 2016 for district 11 the highest number of arrests came from narcotics (3634) and then battery (635).
```{r}

query <- "SELECT primary_type, COUNT(*) AS number_arrests
FROM crime
...
This gives us the same table as before.
```{r}
...
dis_11_arrests_years <- crime_table |>
  ...
  summarise(number_arrests = n(), .groups = "drop") |>
  arrange(desc(number_arrests))
...
https://github.com/pglaser1/Assignment4/blob/ebc0d7e8af1b55d39eef64655135bd823d2e9457/Assignment4_Glaser.Rmd
